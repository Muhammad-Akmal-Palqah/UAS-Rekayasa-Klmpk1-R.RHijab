...
<head>
    ...
    <title>Kelola Admin - R.R HIJAB</title>
...
<body class="bg-gray-50 font-sans antialiased flex flex-col h-screen">
    ...
    <div class="flex flex-1 overflow-hidden">
                    ...
                    <nav class="space-y-1">
                        <div class="space-y-1">
                            ...
                            <div id="masterSubmenu" class="pl-8 space-y-1 mt-1 hidden transition-all duration-200">
                                <a href="{{ route('admin.index') }}" class="flex items-center gap-2.5 px-3 py-2.5 text-sm font-semibold text-gray-900 bg-[#ffcccc]/30 rounded-lg">Admin</a>
        ...
        <main class="flex-1 bg-gray-50 p-8 overflow-y-auto">
            ...
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-3xl font-bold text-gray-800 tracking-tight">Katalog Data Admin</h2>
                <button onclick="toggleModal('addModal')" class="bg-[#ccffcc] hover:bg-[#b3ffb3] text-gray-800 px-4 py-2 rounded-lg text-sm font-semibold flex items-center gap-2 transition-colors shadow-sm">
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-100 text-sm text-gray-700">
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                <td class="py-3.5 px-4 font-medium">{{ $admins->firstItem() + $index }}</td>
                                <td class="py-3.5 px-4 font-semibold text-gray-800">{{ $item->username }}</td>
                                <td class="py-3.5 px-4 text-gray-500">{{ $item->email }}</td>
                                <td class="py-3.5 px-4">
                                    <span class="px-2.5 py-1 rounded-full text-xs font-semibold {{ $item->role === 'super_admin' ? 'bg-purple-50 text-purple-700 border border-purple-200' : 'bg-blue-50 text-blue-700 border border-blue-200' }}">
                                        {{ $item->role }}
                                    </span>
                                </td>
                                <td class="py-3.5 px-4 text-center">
                                    <div class="inline-flex gap-1.5">
                                        <button onclick="openEditModal('{{ $item->id_admin }}', '{{ $item->username }}', '{{ $item->email }}', '{{ $item->role }}')" 
                                                class="bg-[#ccccff] hover:bg-[#b3b3ff] text-gray-800 px-3 py-1 rounded text-xs font-medium shadow-sm transition-colors">
                                            <i class="fa-solid fa-pen-to-square mr-1"></i> Edit
                                        </button>
                                        <form action="{{ route('admin.destroy', $item->id_admin) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus admin ini?')">
                                            @csrf @method('DELETE')
                            ...
                            @empty
                            <tr>
                                <td colspan="5" class="text-center py-8 text-gray-400">Data admin tidak ditemukan.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <div id="addModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm hidden items-center justify-center z-50">
        <div class="bg-white rounded-xl border border-gray-100 shadow-xl max-w-md w-full p-6 mx-4">
            <div class="flex justify-between items-center mb-4 pb-2 border-b">
                <h3 class="text-lg font-bold text-gray-800">Tambah Admin Baru</h3>
                <button onclick="toggleModal('addModal')" class="text-gray-400 hover:text-gray-600"><i class="fa-solid fa-xmark"></i></button>
            </div>
            <form action="{{ route('admin.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="username" required class="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:border-[#ffcccc]">
                </div>
                <div>
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase mb-1">Role Akses</label>
                    <select name="role" required class="w-full px-3 py-2 border rounded-lg text-sm bg-white focus:outline-none focus:border-[#ffcccc]">
                ...
                <button type="submit" class="w-full bg-[#ccffcc] hover:bg-[#b3ffb3] text-gray-800 py-2.5 rounded-lg font-semibold text-sm transition-colors shadow-sm">Simpan Admin</button>
            </form>
        </div>
    </div>
    ...
    <div id="editModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm hidden items-center justify-center z-50">
        <div class="bg-white rounded-xl border border-gray-100 shadow-xl max-w-md w-full p-6 mx-4">
            <div class="flex justify-between items-center mb-4 pb-2 border-b">
                <h3 class="text-lg font-bold text-gray-800">Edit Data Admin</h3>
                <button onclick="toggleModal('editModal')" class="text-gray-400 hover:text-gray-600"><i class="fa-solid fa-xmark"></i></button>
            </div>
            <form id="editForm" method="POST" class="space-y-4">
                @csrf @method('PUT')
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase mb-1">Username</label>
                    <input type="text" id="edit_username" name="username" required class="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:border-[#ffcccc]">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase mb-1">Email</label>
                    <input type="email" id="edit_email" name="email" required class="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:border-[#ffcccc]">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase mb-1">Role Akses</label>
                    <select id="edit_role" name="role" required class="w-full px-3 py-2 border rounded-lg text-sm bg-white focus:outline-none focus:border-[#ffcccc]">
                        <option value="admin">Admin</option>
                        <option value="super_admin">Super Admin</option>
                    </select>
                </div>
                ...
                <button type="submit" class="w-full bg-[#ccccff] hover:bg-[#b3b3ff] text-gray-800 py-2.5 rounded-lg font-semibold text-sm transition-colors shadow-sm">Perbarui Data</button>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        // Fungsi toggle dropdown sidebar dengan efek rotasi chevron icon bawaan
        ...
        function openEditModal(id, username, email, role) {
            document.getElementById('editForm').action = `/admin/manage/${id}`;
            document.getElementById('edit_username').value = username;
            document.getElementById('edit_email').value = email;
            document.getElementById('edit_role').value = role; // // ADDED: Set value pilihan role otomatis sesuai DB
